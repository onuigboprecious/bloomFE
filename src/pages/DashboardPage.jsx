          ...
          <aside className="hidden md:flex flex-col w-64 shrink-0 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-5 space-y-6 shadow-sm sticky top-24">
            ...
            <div className="flex items-center gap-3 pb-4 border-b border-slate-200 dark:border-slate-800/80">
              <img
                src={profile.avatar}
                alt={profile.name}
                className="w-12 h-12 rounded-2xl object-cover border-2 border-cyan-400 shadow-xs shrink-0"
              />
              <div className="space-y-1 min-w-0 flex-1">
                <h3 className="font-extrabold text-sm text-slate-900 dark:text-white truncate">{profile.name}</h3>
                <div className="flex items-center justify-between bg-cyan-500/10 border border-cyan-500/20 px-2 py-0.5 rounded-lg">
                  <span className="text-[10px] font-mono text-[#00BCFF] font-bold truncate">enlazer.app/@{customHandle}</span>
                  <button
                    onClick={handleCopyProfileLink}
                    className="p-0.5 hover:bg-cyan-500/20 rounded text-[#00BCFF] transition-colors cursor-pointer shrink-0"
